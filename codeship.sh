...
function checkFor() {
  command -v $1
  if [ $? == 1 ]; then
    echo "REQUIRED '$1' is not installed, please see docs or contact info@redline13.com"
    exit 1
  fi
}
...
# Capture the command passed in
export COMMAND=$(eval echo $*)
echo "API CALL USED -- $COMMAND"
